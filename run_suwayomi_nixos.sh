...
#  echo "FNM available. Using fnm to manage Node version."
 
#   if [ $? -ne 0 ]; then
#  echo "Error: Failed to switch Node.js version to 22.12.0 using FNM. Exiting."
#       exit 1
#   fi
#   fi

#   echo "Warning: FNM not found or fnm env script not accessible. Ensure FNM is installed and configured correctly."
...
PATH=$(fnm env | grep 'export PATH=' | sed 's/export PATH=//g' | sed 's/"//g' | sed 's/;//g'):$PATH npx vite --version
if [ $? -ne 0 ]; then
